...
{
    int start = editor()->SendScintilla(QsciScintillaBase::SCI_GETENDSTYLED);//get the latest styled position
    int line = editor()->SendScintilla(QsciScintillaBase::SCI_LINEFROMPOSITION,
            start);//get the line of start
    //start = editor()->SendScintilla(QsciScintillaBase::SCI_POSITIONFROMLINE,
            //line);//set the start as the start of line
    qDebug()<<"handleStyleNeeded start is "<<start<<" pos is<<"<<pos;
    if (start != pos)
        styleText(start, pos);
}
