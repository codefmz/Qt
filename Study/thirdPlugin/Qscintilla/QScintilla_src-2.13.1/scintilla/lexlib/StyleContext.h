...
public:
	...
	bool Match(const char *s) {
        if (ch != static_cast<unsigned char>(*s))// if the current char is match failed,then failed.
			return false;
		s++;
        if (!*s) // if the next char is null, then success.
			return true;
        if (chNext != static_cast<unsigned char>(*s)) //  if the next char is not null and is mactch failed with chNext, then false.
			return false;
		s++;
        for (int n=2; *s; n++) { // compare the rest of chars
