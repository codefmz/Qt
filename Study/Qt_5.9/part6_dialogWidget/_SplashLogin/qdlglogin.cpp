    ...
    , ui(new Ui::dlgLogin)
{
    ...
    qDebug().nospace()<<__FILE__<<"("<< __LINE__<<")"<<__FUNCTION__ <<" -- user = " << m_user << " pwd = " << m_pswd;
}
...
{ //字符串MD5算法加密
    ...
    qDebug().nospace()<<__FILE__<<"("<< __LINE__<<")"<<__FUNCTION__ <<" --  md5 = " << md5;
    return md5;
}
...
void QDlgLogin::mousePressEvent(QMouseEvent* event)
{
    ...
    if (event->button() == Qt::LeftButton)
    {
        ...
        qDebug().nospace()<<__FILE__<<"("<< __LINE__<<")"<<__FUNCTION__ <<" -- globalPos() = " << event->globalPos() << "  pos = " << pos() << " m_lastPos = " << m_lastPos;
    }
