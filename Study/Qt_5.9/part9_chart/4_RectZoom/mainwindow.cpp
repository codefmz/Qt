    ...
    ui(new Ui::MainWindow)
{
    ...
    QList<QAbstractSeries *> series = ui->chartView->chart()->series();
    m_valueLabel = new QLabel(this);

    foreach(auto it, series)
    {
        QXYSeries * se = dynamic_cast<QXYSeries*>(it);
        connect(se, &QLineSeries::hovered, this, &MainWindow::slotPointHoverd);
    }
...
//        QPointF pt=ui->chartView->chart()->mapToValue(point); //转换为图表的数值
        QPointF pt = point;
        QString value = QString::asprintf("%.2f : %.2f",pt.x(), pt.y());
        QFont font = m_valueLabel->font();
        QFontMetrics fm(font);
        int pixelsWidth = fm.horizontalAdvance(value);
        int pixelsHeight = fm.horizontalAdvance(value);
        qDebug().nospace()<<__FILE__<<"("<< __LINE__<<")"<<__FUNCTION__ <<" -- value = " << value;
        m_valueLabel->setMinimumSize(pixelsWidth,pixelsHeight);
