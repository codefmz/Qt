...
void MainWindow::iniData()
{
    ...
    for (int i=0;i<theModel->rowCount();i++)
    {
        ...
        { //不包含最后一列
            qreal qrandValue =randGen->bounded(50.0);;
            qDebug().nospace()<<__FILE__<<"("<< __LINE__<<")"<<__FUNCTION__ <<" -- qrand() = " << qrandValue;
            qreal score=50.0 + qrandValue;//随机数
            avgScore += score;
        ...
        aItem->setFlags(aItem->flags() & flag );//平均分不允许编辑
...
void MainWindow::surveyData()
{
    ...
    labelList << "数学" << "语文" << "英语" << "平均分";
    ui->treeWidget->setHeaderLabels(labelList);
    QList<QTreeWidgetItem*> itemList;
    QTreeWidgetItem * itemTop1 = new QTreeWidgetItem();
    itemTop1->setText(0,"60分以下");
    QTreeWidgetItem * itemTop2 = new QTreeWidgetItem();
    itemTop2->setText(0,"60-69分");
    QTreeWidgetItem * itemTop3 = new QTreeWidgetItem();
    itemTop3->setText(0,"70-79分");
    QTreeWidgetItem * itemTop4 = new QTreeWidgetItem();
    itemTop4->setText(0,"80-89分");
    QTreeWidgetItem * itemTop5 = new QTreeWidgetItem();
    itemTop5->setText(0,"90分以上");
    QTreeWidgetItem * itemTop6 = new QTreeWidgetItem();
    itemTop6->setText(0,"平均分");
    QTreeWidgetItem * itemTop7 = new QTreeWidgetItem();
    itemTop7->setText(0,"最高分");
    QTreeWidgetItem * itemTop8 = new QTreeWidgetItem();
    itemTop8->setText(0,"最低分");
    itemList <<  itemTop1 << itemTop2 << itemTop3 << itemTop4 << itemTop5 << itemTop6 << itemTop7 << itemTop8;
...
//用于横坐标在字符串列表,即学生姓名
    ...
    for (int i=0;i<theModel->rowCount();i++)
    {
        //        categories <<theModel->item(i,colNoName)->text().right(1);
...
//    lineseries->setPointLabelsFormat("@yPoint");
    ...
    chart->legend()->setAlignment(Qt::AlignTop); //图例显示在下方
}
...
{ //绘制饼图
    ...
    for (int i=0;i<=4;i++) //添加分块数据
    {
        ...
        if(item->text(colNo) != "0")
        {
           count++;
           series->append(item->text(0),item->text(colNo).toFloat()); //添加一个饼图分块数据,标签，数值
        }
    }
    qDebug().nospace()<<__FILE__<<"("<< __LINE__<<")"<<__FUNCTION__ <<" -- count = " << count;
    QPieSlice *slice; //饼图分块
    for(int i=0;i <= count;i++) //设置每个分块的标签文字
    {
...
//        QObject::disconnect(marker, SIGNAL(clicked()), this, SLOT(on_chartBarLegendMarkerClicked()));
        QObject::connect(marker, SIGNAL(clicked()), this, SLOT(on_chartBarLegendMarkerClicked()), Qt::UniqueConnection);
    }
}
...
//    QBarCategoryAxis *axisX = new QBarCategoryAxis(); //横坐标
    QBarCategoryAxis *axisYBar = new QBarCategoryAxis(); //横坐标
    axisYBar->append(categories);
    axisYBar->setRange(categories.at(0), categories.at(categories.count()-1));
//    chart->setAxisX(axisX, series);
    chart->setAxisY(axisYBar,series);
//    axisX->setRange(categories.at(0), categories.at(categories.count()-1));
...
{
    qDebug().nospace()<<__FILE__<<"("<< __LINE__<<")"<<__FUNCTION__ <<" -- o " << o << " e = " << e;
