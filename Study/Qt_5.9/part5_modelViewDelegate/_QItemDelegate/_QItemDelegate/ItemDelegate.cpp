...
void ItemDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
  ...
  if (parent() && (qobject_cast<VarTreeView*>(parent())) && index.column() == 1) {
    bool isDouble;
...
//  }
  ...
  if (parent() && (qobject_cast<VarTreeView*>(parent()))) {
...
{
  if (parent() && qobject_cast<VarTreeView*>(parent())) {
    VarTreeView *pVariablesTreeView = qobject_cast<VarTreeView*>(parent());
    VarTreeProxyModel *pVariableTreeProxyModel = pVariablesTreeView->mpVarWidget->varTreeProxyModel;
    QModelIndex sourceIndex = pVariableTreeProxyModel->mapToSource(index);
    VarTreeItem *pVariablesTreeItem = static_cast<VarTreeItem*>(sourceIndex.internalPointer());
    if (index.column() == 1) { // value column
      ...
      pComboBox->setEnabled(!pVariablesTreeItem->getVarDisplayUnit().isEmpty());
//      foreach (QString unit, pVariablesTreeItem->getVarDisplayUnit()) {
//        pComboBox->addItem(Utilities::convertUnitToSymbol(unit), unit);
//      }
      QString displayUnit = pVariablesTreeItem->getVarDisplayUnit();
      QString unit = pVariablesTreeItem->getVarUnit();
      pComboBox->addItem(displayUnit, displayUnit);
      pComboBox->addItem(unit,unit);
...
{
  if (parent() && qobject_cast<VarTreeView*>(parent()) && index.column() == 3) {
    VarTreeView *pVariablesTreeView = qobject_cast<VarTreeView*>(parent());
    VarTreeProxyModel *pVariableTreeProxyModel = pVariablesTreeView->mpVarWidget->varTreeProxyModel;
    QModelIndex sourceIndex = pVariableTreeProxyModel->mapToSource(index);
    VarTreeItem *pVarTreeItem = static_cast<VarTreeItem*>(sourceIndex.internalPointer());
    QComboBox* comboBox = static_cast<QComboBox*>(editor);
    //set the index of the combo box
    int currentIndex = comboBox->findData(pVarTreeItem->getVarDisplayUnit());
    if (currentIndex > -1) {
