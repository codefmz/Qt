...
void QsciScintillaQt::Initialise()
{
    ...
    // selection (ie. X11).
    QObject::connect(QApplication::clipboard(), SIGNAL(selectionChanged()), this,
    ...
    if (on)
    {
        if (!idler.state)
        {
            ...
            if (!timer)
            {
                ...
                QObject::connect(timer, SIGNAL(timeout()), this, SLOT(onIdle()));
            }
